...
export const UsersWithSearch: React.FC = () => {
	...
	const onFinish = React.useCallback(
		({ login, fullName, password }: { login: string; fullName: string; password: string }) => {
			createUser
				.mutateAsync({ body: { login, fullName, password } })
				.then((res) => {
					if (res) {
		...
		<div>
			...
			>
				<Form onFinish={onFinish}>
					<FormItem name={'login'} rules={[{ required: true, message: 'Please input login!' }]}>
					...
					<FormItem name={'password'} rules={[{ required: true, message: 'Please password!' }]}>
