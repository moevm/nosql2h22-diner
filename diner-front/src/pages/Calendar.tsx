...
export const Calendar: React.FC<{
  ...
  shifts: Shift[],
  onShiftChange: (shifts: Shift[] | any ) => void
}> = ({ editMode, header, shifts, onShiftChange }) => {
  const [date, setDate] = React.useState(dayjs());
  const calendarRef = React.useRef<HTMLDivElement>(null);
  const [editingShift, setEditingShift] = React.useState<null | { index: number, init: { cursorY: number, hourFrom: number, hourTo: number }, editingProperty: 'hourFrom' | 'hourTo' }>(null);
  const createShiftMouseDownHandler = React.useCallback((shiftIndex: number) => (event: React.MouseEvent) => {
    if (!calendarRef.current) return;
    if (!editMode) return;
    const { y: calendarY } = calendarRef.current.getBoundingClientRect();
    const shift = shifts[shiftIndex];
    const shiftY = calendarY + 50 + (shift.hourFrom - 8) * 50;
  ...
  const createCellMouseDownHandler = React.useCallback((date: Date) => (event: React.MouseEvent) => {
    if (!calendarRef.current) return;
    if (!editMode) return;
    event.preventDefault();
    const { y: calendarY } = calendarRef.current.getBoundingClientRect();
    const hourFrom = Math.round(((event.clientY - calendarY - 50) / 50 + 8) / 0.5) * 0.5;
  ...
  const handleMouseMove = React.useCallback((event: React.MouseEvent) => {
    ...
    const clamped = Math.min(19, Math.max(8, rounded));
  ...
  React.useEffect(() => {
    ...
    for (let i = 0; i < updatedShifts.length; i++) {
      ...
      const day = Math.round(dayjs(shift.date).startOf('date').unix() - dayjs(shift.date).startOf('week').unix()) / (60 * 60 * 24);
      for (let time = Math.round(shift.hourFrom * 10); time <= Math.round(shift.hourTo * 10); time += 5) {
        if (!colisionMap[day]) colisionMap[day] = {};
        if (colisionMap[day][time]) {
          colisionMap[day][time].hourFrom = Math.min(colisionMap[day][time].hourFrom, shift.hourFrom);
  ...
  const createShiftDoubleClickHandler = React.useCallback((shfitIndex: number) => () => {
    if (!editMode) return;
    onShiftChange((prevShifts: Shift[]) => {
  ...
  return <div>
    <Space>
      <DatePicker value={date as any} onChange={setDate as any} allowClear={false} />
      {header}
    </Space>
    <br />
    <br />
    <div className='calendar' onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} ref={calendarRef}>
      <div className='calendar-header calendar-row'>
        <div className='calendar-cell'>
          Time/Date
        </div>
        {Array(7).fill(0).map((_, dayOffset) => <div key={dayOffset} className="calendar-date calendar-cell" >
          {new Intl.DateTimeFormat('en-US', { day: '2-digit', month: 'short' }).format(date.startOf('week').unix() * 1000 + dayOffset * 1000 * 60 * 60 * 24)}
        </div>)}
      </div>
      {Array(12).fill(0).map((_, hourOffset) => <div className='calendar-row' key={hourOffset}>
        <div className='calendar-cell'>
          {(8 + hourOffset)}:00
        </div>
        {Array(7).fill(0).map((_, dayOffset) => (
          <div
            key={dayOffset}
            className={editMode ? `calendar-cell calendar-cell-editable` : `calendar-cell`}
            onMouseDown={createCellMouseDownHandler(new Date(date.startOf('week').unix() * 1000 + (dayOffset + 1) * 1000 * 60 * 60 * 24))}
          />))}
      </div>)}
      {shifts.map((shift, index) => <div
        ...
        style={{
          top: 50 + (shift.hourFrom - 8) * 50,
          height: (shift.hourTo - shift.hourFrom) * 50,
          left: 100 * (((dayjs(shift.date).startOf('date').unix() - dayjs(shift.date).startOf('week').unix()) / (60 * 60 * 24))) - 2
        }}
      />)}
    </div>
  </div>
