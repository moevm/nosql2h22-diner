...
export const Resource: React.FC = () => {
  ...
  const onFinish = ({ name, amount }: { name: string, amount: number }) => {
    if(!editing)
      updateResource.mutateAsync({ body: {
          id, name, amount,
    ...
    <div>
      ...
      <Form form={form} onFinish={onFinish}>
        ...
        <FormItem name={'amount'}>
          <InputNumber defaultValue={resource?.data ? resource.data?.amount : 0} style={{ width: 250 }} disabled={!editing} /> {RESOURCES_UNIT[resource.data?.unit as number]}
        </FormItem>
      </Form>
      <br />
      <br />
